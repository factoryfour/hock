 ...
 * @type {Function}
 */
const Hock = module.exports = function (options) {
	options = options || {};
...
Hock.prototype.hasRoute = function (method, url, body, headers) {
	...
	const found = this._assertions.filter(request => request.method === method
          && request.url === url
          && request.body === body
...
Hock.prototype.handler = function (req, res) {
	...
	req.on('end', () => {
		...
		} else if (self._assertions[matchIndex].sendResponse(res)) {
